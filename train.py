...
#  data folder
parser.add_argument('-data', type=str, default='/home/aviadp/work/thesis/H-DenseUNet/data/', help='test images')
...
class TestLiTs(data.Dataset):
    ...
    def load_seq_crop_data_masktumor_try(self, Parameter_List):
        ...
        if flip_num == 1:
            ...
            cropp_tumor = np.fliplr(cropp_tumor)
        #
        cropp_tumor = resize(cropp_tumor, (args.input_size, args.input_size, args.input_cols), order=0, mode='edge',
                             cval=0, clip=True, preserve_range=True)
        cropp_img = resize(cropp_img, (args.input_size, args.input_size, args.input_cols), order=3, mode='constant',
                           cval=0, clip=True, preserve_range=True)
    ...
    def __getitem__(self, idx):
        ...
        # if (c0 * c1 * c2 == 0):
        #   continue
        return (X.transpose([2, 0, 1])).astype(np.float32), (Y.transpose([2, 0, 1])).astype(np.int64)
...
def train_model(model, dataGen, criterion, optimizer, num_epochs=25):
    # since = time.time()
    ...
    for epoch in range(num_epochs):
        ...
        epoch_acc = total_correct / len(dataGen)
...
if __name__ == "__main__":
    testSet = TestLiTs(args.data, transform=transforms.Compose([transforms.ToTensor()]))
    testGen = data.DataLoader(testSet, batch_size=1, shuffle=True, num_workers=1)
    ...
    model, val_acc = train_model(model, testGen, loss, opt)
